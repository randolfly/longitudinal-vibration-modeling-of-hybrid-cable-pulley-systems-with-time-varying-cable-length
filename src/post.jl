...
function post_sol(mt::ModelType, mp::ModelParam, sol, export_data=false, title="test")
    ...
    begin
        ...
        sol_eta = zeros(plot_size, mp.N + mp.pulley_num)
        sol_deta = zeros(plot_size, mp.N + mp.pulley_num)
        ...
        for i in 1:plot_size
            ...
            x = sol(t[i])[1:mp.N+mp.pulley_num+1]
            dx = sol(t[i])[mp.N+mp.pulley_num+2:end]

            sol_eta[i, :] = x[1:mp.N+mp.pulley_num]
            sol_xe[i] = x[end]
            sol_deta[i, :] = dx[1:mp.N+mp.pulley_num]
            sol_dxe[i] = dx[end]
...
function post_sol(mt::SimpleModel, mp::ModelParam, sol, export_data=false, title="test")
    # extract sol data into 20000 points
    plt_tspan = (sol.t[1], sol.t[end])
    # plot_size = round(20000 * plt_tspan[2] - 20000 * plt_tspan[1])
    plot_size = 5000
